...
for subcorpus in subcorpora:
    directory = f"5.Filtered\\{subcorpus}"
    public =  f"7_PersonalDataRemoved\{subcorpus}"
    subdir_path = os.path.join(directory, subdir)
    for file in os.listdir(subdir_path):
        if "json" in file:
            try:
                path = os.path.join(subdir_path, file)
                new_file = public + "/" + subdir + f"/{file}"
                if os.path.exists(public + "/" + subdir):   
                    if os.path.exists(new_file):
                        print(file, " exists")
                        continue
                else:
                    os.makedirs(public + "/" + subdir)
                with open(path) as f:
                    data = json.load(f)
                if "text" in data:
                    text = data["text"]
